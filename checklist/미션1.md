...
### 코드리뷰 정리
#### 1. 접근 지시자
  - [X] private List<Question> questions
    - 왜그랬을까 재빨리 붙이자
    ```java
    @Controller
...
#### 2. users 공통 url
  - [X] UserController, @RequestMapping("/users")
...
#### 3. 데이터를 숨기고 함수를 공개하자
 - [X] User 클래스에 현재 인스턴스의 password와 인자로 받은 password를 비교하는 메소드 생성
   ```java
...
#### 4. 정규식은 필요할 때만!
 - [X] 정규식을 통해 예외를 던지는 대신, 예외 발생 시 try-catch로 예외 처리
   ...
   - ? 그럼, 어떻게 예외 처리를 할까?
     ...
     2. 사용자에게 적합한 안내를 해준다(해당 에러와 관련된 뷰를 리턴한다) - 이게 더 좋아보임
       ...
       - `@ExceptionHandler` 사용
   - 😱 ExceptionHandler에서 Model 리턴 불가능
     - Model을 아래와 같이 리턴하려다 실패
     ```java
     ...
     ```
     - Model은 리턴 못하고 ModelAndView만 가능
     - [Controller Based Exception Handling의 Important Note 참고](https://spring.io/blog/2013/11/01/exception-handling-in-spring-mvc)
       
#### 5. 예외 처리
 - [X] Custom Exception을 만들어 사용해보자
 - [리뷰] 기왕에 **명시적으로 예외를 던지기로 했다면** 직접 설계한 예외 객체를 활용해보는 것도 좋을 뻔했네요
    ...
    - [Java Custom Exception Examples 참고](https://mkyong.com/java/java-custom-exception-examples/)
